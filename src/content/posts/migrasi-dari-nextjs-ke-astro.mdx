...
tags: ["Astro", "Next.js", "Migration", "Performance", "Cloudflare", "Web Development", "Static Site Generator", "Tutorial"]
draft: false
...
Setiap kali saya ingin publish artikel baru, proses build di Vercel memakan waktu 2-3 menit. Untuk blog dengan 10-15 artikel, ini terasa berlebihan.
...
### 2. Konfigurasi Content Collections

Salah satu fitur terbaik Astro adalah Content Collections yang memberikan type safety untuk konten markdown:
...
## Hasil Migrasi: Analisis Performa

### Metrics Sebelum (Next.js)
...
### Metrics Sesudah (Astro)
- **Build Time**: 30-45 detik
...
**Masalah**: Beberapa komponen React memerlukan dynamic imports.

**Solusi**: Menggunakan `client:visible` directive untuk lazy loading:
...
**Masalah**: Sharing state antar komponen tanpa React Context.

**Solusi**: Menggunakan Nano Stores untuk state management yang ringan:
...
Blog ini sekarang bisa diakses di [febryan.web.id](https://febryan.web.id) dengan performa yang jauh lebih baik. Link lama di [fepryan.github.io/portfolio](https://fepryan.github.io/portfolio) masih aktif, tapi semua update terbaru akan ada di domain baru.
