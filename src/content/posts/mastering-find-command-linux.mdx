...
tags: ['linux', 'find']
draft: false
summary: 'Complete hands-on guide to mastering the find command and -exec option - from basic file searches to advanced logical operators, -prune optimization, and production-ready automation patterns. Learn through real experiments and avoid common pitfalls. | Panduan lengkap menguasai command find dan option -exec - dari pencarian file dasar hingga logical operators advanced, optimasi -prune, dan pattern automation untuk production. Belajar melalui eksperimen nyata dan hindari kesalahan umum.'
thumbnail: "/images/blog/find-exec-mastery-thumb.webp"
...
openGraph:
  ...
  url: "https://febryan.web.id/blog/mastering-linux-find-exec"
twitter:
  card: "summary_large_image"
  title: "Master Linux find & -exec 🔍🚀"
...
# Mastering Linux `find` and `-exec`: From Basics to Expert Patterns
...
- `-mtime 30` → Modified **exactly** 30×24 hours ago (range: 30-31 days)
- `-mtime +30` → Modified **more than** 31 days ago (older files)
- `-mtime -30` → Modified **less than** 30 days ago (newer files)
...
**Breaking it down:**
1. `-type f` → Only files
2. `\( ... \)` → Group the OR conditions
3. `-name "*.log" -mtime +90` → .log files older than 90 days
4. `-o` → OR
5. `-name "*.gz" -mtime +180` → .gz files older than 180 days
6. (implicit AND) `-size +100M` → AND size > 100MB
...
**Logic flow:**
1. `\( -name skip_dir \) -prune` → If directory name matches, prune (don't descend)
2. `-o` → OR
3. `-type f -name "*.log" -print` → Otherwise, if it's a .log file, print it
...
/tmp/prunetest/main.log
/tmp/prunetest/backup       ← Why is this here?
/tmp/prunetest/app/debug.log
/tmp/prunetest/app/error.log
/tmp/prunetest/archive      ← Why is this here?
```

**What happened?**

When `find` encounters `backup/` directory:
1. Test: `-name backup` → **TRUE**
2. Action: `-prune` → Executed (don't descend)
3. Return: **TRUE**
...
# ❌ AMBIGUOUS (directories will also print)
...
- ✅ Use `-exec {} +` (batch mode) - can be 100x faster
- ❌ Avoid `-exec {} \;` unless you need per-file execution
- ⚠️ Remember: `{}` must be standalone with `+`
...
- `-mtime +N` → **Older** than N days (towards PAST)
- `-mtime -N` → **Newer** than N days (towards NOW)
